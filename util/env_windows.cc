// This file contains source that originates from under the "New BSD License":
// https://bitbucket.org/robertvazan/leveldb-windows/src/7f1fbfca6fc06f31323176de56720684c26ca54b/util/env_win.cc?at=default&fileviewer=file-view-default
//
// This file in turn contains source that originates from:
...
#define DISALLOW_COPY_AND_ASSIGN(TypeName) \
  ...
  void operator=(const TypeName&)
		...
		void ToWidePath(const std::string& value, std::wstring& target) {
			...
			target.resize(size_needed);
			MultiByteToWideChar(CP_UTF8, 0, &value[0], (int)value.size(), &target[0], size_needed);
		}
		...
		void ToNarrowPath(const std::wstring& value, std::string& target) {
			...
			target.resize(size_needed);
			WideCharToMultiByte(CP_UTF8, 0, &value[0], (int)value.size(), &target[0], size_needed, NULL, NULL);
		}
		...
		std::string GetCurrentDir()
		{
			...
			*wcsrchr(path, '\\') = 0;
			...
			ToNarrowPath(std::wstring(path), narrow_path);
			return narrow_path;
		}
		...
		bool Win32MapFile::_MapNewRegion()
		{
			...
			_base_handle = CreateFileMappingW(
				_hFile,
				NULL,
				PAGE_READWRITE,
				0,
...
#define GetTickCount64 GetTickCount
		...
		Status Win32Env::CreateDir(const std::string& dirname)
		{
			...
			GetFullPathName(path.c_str(), MAX_PATH, szAbsolutePath, NULL);
			if (!::SHCreateDirectoryExW(NULL, szAbsolutePath, NULL)) {
